...
class MDPGrid(object):
    ...
    def plot_Q(self, axes, Q, print_numbers=False):
        """
        Plot the Q-values for each state-action pair as small triangles inside the grid.

        Args:
            axes: The axes object for plotting.
            Q: A dictionary mapping state-action pairs to Q-values (Q[state][action]).
            print_numbers: Whether to print the Q-value numbers inside the triangles.
        """
        colormap = plt.get_cmap('bwr_r')

        # Get overall range of Q-values
        maxval = max(q_val for q_values in Q.values() for q_val in q_values.values())
        minval = min(q_val for q_values in Q.values() for q_val in q_values.values())

        font0 = FontProperties()
        font1 = font0.copy()
        font1.set_size('small')

        # Define the four actions with corresponding triangle positions
        action_triangles = {
            'up': lambda cx, cy, w, h: [(cx - 0.5 * w, cy + 0.5 * h), (cx + 0.5 * w, cy + 0.5 * h), (cx, cy)],  # Flipped to point down
            'down': lambda cx, cy, w, h: [(cx - 0.5 * w, cy - 0.5 * h), (cx + 0.5 * w, cy - 0.5 * h), (cx, cy)],  # Flipped to point up
            'left': lambda cx, cy, w, h: [(cx - 0.5 * w, cy - 0.5 * h), (cx - 0.5 * w, cy + 0.5 * h), (cx, cy)],
            'right': lambda cx, cy, w, h: [(cx + 0.5 * w, cy - 0.5 * h), (cx + 0.5 * w, cy + 0.5 * h), (cx, cy)]
        }

        # Loop over each state and plot the Q-values
        for state, q_values in Q.items():
            centerx, centery = self.cell_xy(*state)
            width, height = self.cell_dimensions

            # Create and plot triangles for each action in the state cell
            for action, q_val in q_values.items():
                # Generate triangle vertices based on the action
                triangle = action_triangles[action](centerx, centery, width, height)

                # Normalize the Q-value to the range [0, 1]
                # power = float(0.5*q_val/maxval + 0.5)
                if maxval != 0:
                    power = float(0.5 * q_val / maxval + 0.5)
                else:
                    power = float(0.5 * q_val / 100 + 0.5)

                # Create polygon for the action and color it based on Q-value
                triangle_patch = Polygon(triangle, closed=True)
                triangle_patch.set_facecolor(colormap(power))
                axes.add_patch(triangle_patch)

                # Optionally print the Q-value inside the triangle
                if print_numbers:
                    if action == 'up':
                        text_x, text_y = centerx, centery + 0.25 * height  # Position for 'up'
                    elif action == 'down':
                        text_x, text_y = centerx, centery - 0.25 * height  # Position for 'down'
                    elif action == 'left':
                        text_x, text_y = centerx - 0.25 * width, centery  # Position for 'left'
                    elif action == 'right':
                        text_x, text_y = centerx + 0.25 * width, centery  # Position for 'right'

                    axes.text(text_x, text_y, "%.2f" % q_val, fontproperties=font1,
                              horizontalalignment='center', verticalalignment='center')

        # Normalize and add colorbar
        norm = mpl.colors.Normalize(vmin=minval, vmax=maxval)
        colormap_axes = plt.gcf().add_axes([0.9, 0.2, 0.05, 0.7])
        mpl.colorbar.ColorbarBase(colormap_axes, cmap=colormap, norm=norm, orientation='vertical')
...
def generate_grid_plot(mdp, obstacles, goal):
    n = np.sqrt(len(mdp.S))
    g = MDPGrid(n,n)
    axes = g.draw()
    g.draw_cell_circle(axes, goal, color='g')
